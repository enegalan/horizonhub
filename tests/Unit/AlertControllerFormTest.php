...
class AlertControllerFormTest extends TestCase
{
    use RefreshDatabase;

    public function test_form_rule_types_exclude_removed_job_failure_variants(): void
    {
        $vars = $this->private__buildFormViewVariables(new Alert);
    ...
    public function test_form_services_include_disabled_services_still_in_alert_scope(): void
    {
        ...
        $services = $this->private__buildFormViewVariables($alert)['services'];
    ...
    {
        $controller = $this->app->make(AlertController::class);
        $reflection = new \ReflectionMethod($controller, 'private__buildFormViewVariables');
        $reflection->setAccessible(true);

        return $reflection->invoke($controller, $alert);
    }
}
